...
\tikzset{
  vertex/.style={circle, draw, fill=gray!10, inner sep=2pt},
  weight/.style={draw=none, fill=none, inner sep=1pt, font=\small}
}

\node[vertex] (a) at (0,0) {A};
\node[vertex] (b) at (1.5,1) {B};
\node[vertex] (c) at (3,0.8) {C};
\node[vertex] (d) at (0.5,-1.5) {D};
\node[vertex] (e) at (2.5,-1.2) {E};

\draw (a)--node[weight, above left]{2}(b);
\draw (b)--node[weight, above]{3}(c);
\draw (a)--node[weight, left]{6}(d);
\draw (b)--node[weight, right]{8}(d);
\draw (b)--node[weight, right]{5}(e);
\draw (c)--node[weight, right]{7}(e);
\draw (d)--node[weight, below]{9}(e);
\end{tikzpicture}
